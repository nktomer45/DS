...
function caesarCipher(str, shift) {
    ...
    const alphetic="abcdefghijklmnopqrstuvwxyz".split('')
    let res=''
    for(let i=0;i<stringA.length;i++){
        let inID=alphetic.indexOf(stringA[i])
        if(inID===-1){
            console.log(inID)
            res+=stringA[i]
            continue;
        }
        let inshift=(inID+shift)%26
        console.log(inshift,alphetic[inshift])

        res+=alphetic[inshift]
